...
#BOOT_SCRIPT="/usr/share/debootstrap/scripts/noble"
BOOT_SCRIPT=""
...
try_unmount_prototype () {
	...
	umount "${IMAGE_DIR}/isolinux/efi" || umount -lf "${ROOTFS}/isolinux/efi" || true
}
...
debootstrap --variant=minbase --components=main,universe,restricted,multiverse \
	...
	"${CODENAME}" "${ROOTFS}" "${MIRROR}" "${BOOT_SCRIPT}"
...
cat << __EOF__ > "${IMAGE_DIR}/isolinux/grub.cfg"
...
#grub-mkimage -o "${IMAGE_DIR}/isolinux/core.img" -O i386-pc -p /boot/grub biosdisk ext2 fat iso9660 search
...
echo "Creating BIOS boot image on iso_structure/isolinux/bios.img ..."
cat /usr/lib/grub/i386-pc/cdboot.img "${IMAGE_DIR}/isolinux/core.img" > "${IMAGE_DIR}/isolinux/bios.img"
...
echo "Creating EFI boot image on iso_structure/isolinux/efiboot.img ..."
...
find . -type f -print0 \
	| xargs -0 md5sum \
	| sed 's|^\./||' \
	| grep -v -E '(^md5sum.txt$|/boot/grub/i386-pc/grub-standalone.img)' \
	> md5sum.txt
