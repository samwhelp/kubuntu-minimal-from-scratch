...
try_unmount_prototype () {
	...
	umount "${IMAGE_DIR}/boot/efi" || umount -lf "${ROOTFS}/boot/efi" || true
}
...
##
## ## Prepare Chroot Script Asset
##

mkdir -p "${ROOTFS}/opt/build"

[ -d "${BASE_DIR_PATH}/asset" ] && cp -rfT "${BASE_DIR_PATH}/asset" "${ROOTFS}/opt/build/asset"
...
cp -rfT /opt/build/asset/overlay /
...
if [ -x "/opt/build/asset/hook/main.sh" ]; then
	"/opt/build/asset/hook/main.sh"
fi
...
apt-get install -y --install-recommends dconf-cli
